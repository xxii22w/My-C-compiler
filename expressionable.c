...
void expressionable_node_push(struct expressionable* expression,void* node_ptr)
{
    vector_push(expression->node_vec_out,&node_ptr);
}
...
void expressionable_ignore_nl(struct expressionable* expressionable,struct token* next_token)
{
    while(next_token && token_is_symbol(next_token,'\\'))
    {
        ...
        assert(nl_token->type == vector_peek(expressionable->token_vec));
...
{
    memset(expressionable,0,sizeof(expressionable));
...
void expressionable_parse_for_indirection_unary(struct expressionable* expressionable)
{
    ...
    expressionable_callbacks(expressionable)->make_unary_indirection_node(expressionable,depth,unary_operand_compatible);
}
...
int expressionable_parse_token(struct expressionable* expression,struct token* token,int flags)
{
    int res = -1;
    switch (token->type) 
    {
        ...
        case TOKEN_TYPE_NUMBER:
            expressionable_parse_number(expression);
            res = 0;
            break;

        case TOKEN_TYPE_IDENTIFIER:
            expressionable_parse_identifier(expression);
            res = 0;
            break;

        case TOKEN_TYPE_OPERATOR:
            expressionable_parse_exp(expression, token);
            break;
    }
    return res;
}
